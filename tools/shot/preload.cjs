...
const settings = {
  ...
  density: 'normal', showToggles: true, claudeLimit5h: 0, claudeLimit7d: 0
}
...
window.aicycle = {
  ...
  quit: () => {},
  onSnapshots: () => () => {},
  // The real main process pushes settings it changed itself (the context menu).
  // Keep the callback reachable so a harness can drive that path.
  onSettings: (cb) => { window.__pushSettings = cb; return () => { window.__pushSettings = null } },
  onOpenSettings: () => () => {}
}
